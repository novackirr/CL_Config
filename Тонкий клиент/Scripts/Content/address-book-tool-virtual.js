...
var removeAllAddressBookVirtual = function (removeGlyph, registerName) {
...
var removeItemAddressBookVirtual = function (removeGlyph, registerName) {
    var dictControl = $(removeGlyph).closest(".dict-modal-control");
...
var AddressBookToolVirtual = function (buttonElement, selectedElements) {
                    ...
                    selectedRowsDebug.forEach(function (r) {
                        if (r.get('Key') !== node.data.Key) {
                            if (r.set) {
                                ...
                                var treeIndex = $(".x-tab.x-tab-active").length > 0 && trees.length > 1 ? $(".x-tab").index($(".x-tab.x-tab-active")) : 0;
                                trees[treeIndex].tree.view.refreshNode(r);
                            }
                        }
                    });
                ...
                //},
                {
                    ...
                    listeners: {
                        click: function (ctrl, t, rowIndex) {
                            ...
                            if (!row.data.Selectable && !checked)
                                return false;
        ...
        var grid = Ext.create('Ext.grid.Panel', {
            ...
            listeners: {
                ...
                beforeitemclick: function (view, record, item, index, e, eOpts) {
                    if (!record.data.Selectable && !record.data.Checked) {
                        return false;
                    }
                },
            },
    ...
    var itemClickHandler = function (node, record, item, index, e, eOpts) {        
        if (e.target.className.indexOf("x-tree-checkbox") == -1 && $(e.target).find(".x-tree-checkbox").length == 0) {            
                    ...
                    function setSelectedItems() {
                        ...
                        if (storebookTpl.length > 0) {
                            ...
                            $('<div name=\'removeItemsFooter\'><hr class="action-separator"/>'
                                ...
                                + '<button title="очистить список" id="removeItemsAll-bottom" type="button" class="btn btn-default action-container--delete" onclick="removeAllAddressBookVirtual(this, \''
                            ...
                            for (var i = 0; i < storebookTpl.length; i++) {
                                ...
                                var removeElement = $('<div onclick="removeItemAddressBookVirtual(this, \''
        ...
        var store = Ext.create('Ext.data.Store', {
            ...
            listeners: {
                load: function (store, records, success, operation) {
                    ...
                    var isFind = false;
                    var treeKey = store.id.replace("adrList", "") * 1;
