$(document).ready(function () {
    ...
    function filter(word) {
        var flowItems = $('#menuFlows>input');

        var shown = 0;
        for (var i = 0; i < flowItems.length; i++) {
            var flEl = flowItems[i];
            //var flowId = $(flEl).data('flow-id');
            var flName = $(flEl).val().trim().toLowerCase();

            if (flName.indexOf(word) === -1) {
                $(flEl).hide();
            } else {
                $(flEl).show();
                shown++;
    ...
    $("#basedForms").change(function () {
        basedFormSelected();
    });

    function basedFormSelected() {
        selectedFormId = $('#basedForms').val();
        loadFlowDataByFormId(selectedFormId);
    }

    $('input[type=radio][name=editMode]').change(function () {
        $('#flowSelectBlock').hide();
        $('#baseFormBlock').hide();
        $('#editBlock').hide();
        editor.setValue('');

        if (this.value == 'editForm') {
            isEditMode = true;
            $('#flowSelectBlock').show();
        }
        else if (this.value == 'newForm') {
            isEditMode = false;
            $('#baseFormBlock').show();

            if (selectedFormId) loadFlowDataByFormId(selectedFormId);
        }
    });

    $("#searchFlow").on('input propertychange',function () {
        filter(search.value.trim().toLowerCase())
    });

    $("#menuFlows").change(function () {
        selectedFormId = this.value;
    });    

    $('#menuFlows').on('click', '.dropdown-item', function () {
        $("#dropdown_flows").dropdown('toggle');
        $('#dropdown_flows').text($(this).val());
        selectedFormId = $(this).data('flow-id');
        if (selectedFormId) {
            loadFlowDataByFormId(selectedFormId);
        }
    });

    function loadFlowData(param) {
        waitingDialog.show("Загрузка данных потока...");

        $.ajax({
            url: getRelativeUrl("FormDesigner/GetFlowData?" + param),
            type: "GET",
            cache: false,
            success: function (res) {
                var flowDefinition = res.FlowDefinition;
                editor.setValue(flowDefinition);
            ...
            error: function (error) {
                waitingDialog.hide();
                showCommonErrors(JSON.stringify(error));
            }
        });
    }

    //function loadFlowDataByFlowId(flowId) {
    //    loadFlowData("flowId=" + flowId);
    //}

    function loadFlowDataByFormId(formId) {
        loadFlowData("formId=" + formId);
    }

    $('#btnSaveFlowData').on('click', function () {
        var newFormName = $.trim($('#newFormName').val());

        if (!isEditMode) {
            if (!selectedFormId) {
                alert('Не указана базовая форма');
                return;
            }

            if (!newFormName) {
                alert('Не указано название новой формы');
                return;
            }
        }


        if (confirm('Сохранить/обновить данные формы в БД?')) {
            //var contentToModify = $('#flowSchemaContent').val();
            var contentToModify = editor.getValue();

            waitingDialog.show("Сохранение данных потока...");
            $.ajax({
                url: getRelativeUrl("FormDesigner/SaveFlowData"),
                type: "POST",
                cache: false,
                data: {
                    flowSchema: contentToModify,
                ...
                success: function (res) {
                    if (res.success) {
                        ...
                        showCommonErrors(JSON.stringify(truncateLongText(res.error)));
                    }
                ...
                error: function (error) {
                    waitingDialog.hide();
                    showCommonErrors(JSON.stringify(truncateLongText(error)));
                }
            });
        }
    });
    ...
    function truncateLongText(str) {
        return (str && str.length > 300) ? str.substr(0, 300 - 1) + '&hellip;' : str;
    };

    $('#btnResetFlowData').on('click', function () {

        var parameter;
        if (selectedFormId) parameter = "formId=" + selectedFormId;

        if (parameter) {
            loadFlowData(parameter);
        }
    });

    $("input[name='editMode']#editForm").prop('checked', true);
    $("input[name='editMode']#editForm").change();

    $('#newFormName').on('input', function (e) {
        var input = $(this);
        var val = input.val();
        if (!val) {
