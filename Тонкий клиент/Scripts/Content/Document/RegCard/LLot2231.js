...
var editreg = function() {
	...
	$("button[name='RukUpr']").attr('disabled', true);
...
var okpdhide = function() {
	...
	$("[name='noAddNoPublic'][type='checkbox']").show();
	$("[data-related-field=noAddNoPublic]").show();
...
function proverkaOKPD2forMSP () {
	$(document).on('change', "input[name*='ItemTab-registerOKDP-']", function (e) {
		var registerSpZakup = $("input[name='registerSpZakup']");
		if ((TRUDict[e.target.value]) && $("input[data-field-name='registerUchZakSMP']").prop("checked", false) && registerSpZakup.val()){
			...
			$("input[data-field-name='ItemTab-registerOKDP-" + row +"']").clear();
			$("input[name='ItemTab-registerOKDP-" + row +"']").clear();
			$("input[name='ItemTab-positionDirectoryName-" + row +"']").clear();
		}
	})
}
	...
	$("input[name='registerPlDateOI']").parent().on('dp.change', function (e) {
		...
		if ($("input[name='registerPlDateOI']").val() !="") {
			if (today1Spec > periodOfExecution) {
				...
				$("input[name='registerPlDateOI']").clear();
				return;
			}
		}
		...
		if (date.year()<currentYear){
			showCommonErrors('Год исполнения договора не может быть раньше текущего года');
			$("input[name='registerPlDateOI']").clear();
			return;
		}*/
		if(registerPlDateOI ==""){
...
var hideetpreg = function() {
    ...
    registerZakElForm.change(function() {
        if ($(this).is(":checked")) {
			...
			naimETPName.closest(".clearfix").find(".dict-display-field").prop("required", true);
            ...
            naimETPName.closest(".clearfix").find(".dict-display-field").prop("required", false);
			naimETPName.closest(".column-container").find(".documentView-field-label").removeClass("label-required");
			naimETPName.closest(".clearfix").find(".dict-display-field").val("");
...
//на регистрацию Номер извещения на ЭТП
var hidenumprocdureetpreg = function() {
	var NumProcedureETP=$("input[name='registerNomberETP']");	
	var NumLot=$("input[name='registerNomberLotinIzv']");
	
	NumProcedureETP.closest(".column-container").hide();
	NumProcedureETP.val("");
	NumLot.closest(".column-container").hide();
	NumLot.val("");
}	
//на редактирование Номер извещения на ЭТП
var hidenumprocdureetpedit = function() {
	var flag = $("#editView input[name='registerZakElForm']");
	var NumProcedureETP=$("#editView input[name='registerNomberETP']");
	var NumLot=$("#editView input[name='registerNomberLotinIzv']");
	flag.change(function() {
        if ($(this).is(":checked")) {
			NumProcedureETP.closest(".column-container").show();
			NumLot.closest(".column-container").show();
		} else {
			NumProcedureETP.closest(".column-container").hide();
			NumProcedureETP.val("");	
			NumLot.closest(".column-container").hide();
...
$("div[data-name='ItemTab']").on('change', "input[data-field-name^='ItemTab-NevCol-']", function () {
	...
	if (id != undefined) {
		if (current.is(":checked")) {
			...
			$("input[data-field-name='ItemTab-summaTax-" + id + "']").clear();
...
var WinnerHide = function () {
	...
	else if (spzak != "26646" || spzak != "32323" || spzak != "3363" || spzak != "58932" || spzak == "211246" || spzak == "410218"){
		$("li:has(:contains('Контрагент'))").hide();
...
function changeOKPDforMSP() { // Чекбокс "Участник закупки МСП" в зависимости от "Способа закупки" и контрагента
	...
	} else if (spzak == "3363" || spzak == "240548" || spzak == "2464" || spzak == "33505" || spzak == "103765" || spzak == "285643") {
...
$(document).on('change', "[name='CMP'][type='checkbox']", function (e) {
	var spzak=$("input[name='registerSpZakup']").val();
	var nameOrg = $("input[name='registerOrgZa']");
		if (spzak == "211246" || spzak == "3363" || spzak == "433946" || spzak == "516291") {
			SupplierMSPLogic(); // логика работы, если признак мсп=1 у поставщика 
			TableRowsValidaion(); // Проверка принадлежности позиций к признаку МСП
	}
})
...
$(document).on('click', "#registerSpZakup", function (e){
    $("input[name='registerSpZakup']").data('val', $("input[name='registerSpZakup']").val());//записываем старое значение, чтобы потом ↓ прописать условие  - если поменяли с мсп на немсп, то чистим registerOKDP
}).on('change', "input[name='registerSpZakup']", function(){
/* 	let table = $("div[data-name='ItemTab']")
	let spzak = $("input[name='registerSpZakup']").val()
	let prev = $("input[name='registerSpZakup']").data('val');
	table.find("div.table-edit-row[data-rowkey]").each(function () {
		let i = $(this).attr("data-rowkey")
		if ( (spzak != "200608" && spzak != "200609" && spzak != "200610" & spzak != "200611") ) {
			$(this).find("input[name='ItemTab-OKDPUMSP-"+i+"']").clear()
			
			if ( (prev == "200608") || (prev == "200609") || (prev == "200610") || (prev == "200611") ) {
				$("input[data-field-name^='ItemTab-registerOKDP-']").clear();
				$("input[name^='ItemTab-registerOKDP-']").clear();
			}
		}
	}); */
	proverkaNMCRub();
...
var trebkuch = function() {    
    var Status_view = $(".documentView-field-value[data-name='Статус']").text();
	var ChangeView = $("li:has(:contains('Требования к участникам'))");	
	var zay = $("li:has(:contains('Заявка на размещение закупки'))");
    var  ChangeStatuses = [
...
var torgedin = function() {    
	var flag=$("input[data-field-name='TorgiEd']");
	if ($(flag).is(":checked")) {
		$("input[data-field-name='NMK']").closest(".column-container").show();
		$("div[data-related-field='NMK']").closest(".column-container").show();
		$("input[data-field-name='NMK']").prop('required', true);
		$("div[data-related-field='NMK']").addClass("label-required");
	} else {
		$("input[data-field-name='NMK']").closest(".column-container").hide();
		$("div[data-related-field='NMK']").closest(".column-container").hide();
		$("input[data-field-name='NMK']").prop('required', false);
		$("div[data-related-field='NMK']").removeClass("label-required");
	}
}
	
$(document).on('change', "input[data-field-name='TorgiEd']", function (e) {
	var flag=$("input[data-field-name='TorgiEd']");
	if ($(flag).is(":checked")) {
		$("input[data-field-name='NMK']").closest(".column-container").show();
		$("div[data-related-field='NMK']").closest(".column-container").show();
		...
		$("input[data-field-name='NMK']").prop('required', false);
		$("div[data-related-field='NMK']").removeClass("label-required");
		$("input[data-field-name='NMK']").val('');
	}
});

var torgedinView = function () {
	var flag=$("div[data-name='Торги за единицу товара']").find("input[type='checkbox']");
	if ($(flag).attr("checked")) {	
		$("div[data-name='Начальная цена комплекта']").closest(".column-container").show(); 
		$("div[data-name='Торги за единицу товара']").closest(".column-container").show(); 
	} else {
		$("div[data-name='Начальная цена комплекта']").closest(".column-container").hide(); 
		$("div[data-name='Торги за единицу товара']").closest(".column-container").hide(); 
	}
}
...
$(document).on('change', "input[name='Currency_dig_kod']", function (e) {
	...
	if (flag!=="RUB") {
		...
		/* $("div[title='Год']").attr("style", "width: 150px; min-width: 27.3%;");
		$("input[data-field-name*='planPayment-yearPayment']").parent().parent().attr("style", "width: 150px; min-width: 27.3%;");
		$("div[title='Сумма платежа']").attr("style", "width: 230px; min-width: 36.3%;");
		$("input[data-field-name*='planPayment-summaPayment']").parent().parent().attr("style", "width: 230px; min-width: 36.3%;");
		$("div[title='Сумма платежа в рублевом эквиваленте']").attr("style", "width: 230px; min-width: 36.4%; display: inline-block;");
		$("input[data-field-name*='planPayment-summaPaymentRub']").parent().parent().attr("style", "width: 230px; min-width: 36.4%;"); */
		...
		/* $("div[title='Год МСП']").attr("style", "width: 150px; min-width: 27.3%;");
		$("input[data-field-name*='planPaymentSMP-yearPaymentSMP']").parent().parent().attr("style", "width: 150px; min-width: 27.3%;");
		$("div[title='Сумма платежа МСП']").attr("style", "width: 230px; min-width: 36.3%;");
		$("input[data-field-name*='planPaymentSMP-summaPaymentSMP']").parent().parent().attr("style", "width: 230px; min-width: 36.3%;");
		$("div[title='Сумма платежа в рублевом эквиваленте МСП']").attr("style", "width: 230px; min-width: 36.4%; display: inline-block;");
		$("input[data-field-name*='planPaymentSMP-summaPaymentRubSMP']").parent().parent().attr("style", "width: 230px; min-width: 36.4%;"); */
	 } else {
		$("input[data-field-name='NMCD']").closest(".column-container").hide();
		$("div[data-related-field='NMCD']").closest(".column-container").hide();
		$("div[data-related-field='emptyLabel']").closest(".column-container").hide();
		...
		/* $("div[title='Год']").attr("style", "width: 240px; min-width: 27.3%;");
		$("input[data-field-name*='planPayment-yearPayment']").parent().parent().attr("style", "width: 240px; min-width: 27.3%;");
		$("div[title='Сумма платежа']").attr("style", "width: 370px; min-width: 36.3%;");
		$("input[data-field-name*='planPayment-summaPayment']").parent().parent().attr("style", "width: 370px; min-width: 36.3%;"); */
		...
		/* $("div[title='Год МСП']").attr("style", "width: 240px; min-width: 27.3%;");
		$("input[data-field-name*='planPaymentSMP-yearPaymentSMP']").parent().parent().attr("style", "width: 240px; min-width: 27.3%;");
		$("div[title='Сумма платежа МСП']").attr("style", "width: 370px; min-width: 36.3%;");
		$("input[data-field-name*='planPaymentSMP-summaPaymentSMP']").parent().parent().attr("style", "width: 370px; min-width: 36.3%;"); */
	}
...
function changeCurs(){
	$(document).on('change', "input[name='curs']", function (e) {
		...
		SummaOsnTestSMP.Calculate()
		SumSMPRub.CalculateSumma()
		proverkaNMCValuta(); // проверка на непревышение суммы в валюте у определенных способов
	});
}
...
/* $(document).on('change', "input[data-field-name='curs_dolg']", function (e) {
	SumDolgRub.CalculateSumma(this);
}); */
...
/* $(document).on('change', "input[data-field-name='curs_smp']", function (e) {
	SumSMPRub.CalculateSumma(this);
}); */
...
var ZakupNesost = function () {
	var flag=$("div[data-name='Закупка не состоялась']").find("input[type='checkbox']");
	if ($(flag).attr("checked")) {	
		$("div[data-name='Дата признания несостоявшейся']").closest(".column-container").show(); 
		$("div[data-name='Причина признания несостоявшейся']").closest(".column-container").show(); 
	} else {
		$("div[data-name='Дата признания несостоявшейся']").closest(".column-container").hide(); 
		$("div[data-name='Причина признания несостоявшейся']").closest(".column-container").hide(); 
	}
}
...
var Uchet = function(){
	var data= $("input[name='datepostnauch']")
	if (data.val()!="") {
		$("input[data-field-name='orgpostdogRegRus']").prop('checked', true);
		$("input[data-field-name='orgpostdogRegRus']").prop('readonly', true);
	} else {
		$("input[data-field-name='orgpostdogRegRus']").prop('checked', false);
	}
...
/* $(document).on('change', "input[name='smpdictionary']", function (e) {
	SMP();
}); */

/* $(document).on('change', "input[name='neresdictionary']", function (e) {
	NeresidentfromDictionary();
});
 */
$(document).on('change', "input[data-field-name='datepostnauch']", function (e) {
	Uchet();
});
...
var SPZakrequired = function() {
	var registerSpZakup=$("input[name='registerSpZakup']");		
	registerSpZakup.closest(".clearfix").find(".dict-display-field").prop("required", true);
	registerSpZakup.closest(".column-container").find(".documentView-field-label").addClass("label-required");
...
var osnEP = function () {
	var spzak = $("input[name='registerSpZakup']").val();
	if (spzak == "240548" || spzak == "2464" || spzak == "33505" || spzak == "3363" || spzak == "103765" || spzak == "285643"){
...
var osnPB = function () {
	var spzak = $("input[name='registerSpZakup']").val();
	if (spzak == '202506'){
		$("input[data-field-name='osnPB']").closest(".column-container").show();
		$("[data-related-field=osnPB]").show();
	}
	else{
		$("input[data-field-name='osnPB']").closest(".column-container").hide();
		$("[data-related-field=osnPB]").hide();
		$("input[data-field-name='osnPB']").closest(".clearfix").find(".dict-display-field").val("");
		$("input[name='osnPB']").val('');
		$("textarea[data-field-name='osnPBName']").text('');
		$("textarea[data-field-name='osnPBName']").val('');
		$("input[name='osnPBName']").attr('value','');
	}
}

var osnPBEPview = function () {
	var flag = $(".documentView-field-value[data-name='Способ закупки']").attr("title");
	var osnEP = $("div[data-name='Основание закупки у ЕП']");
	if(flag != undefined){
		if (flag.indexOf("у единственного") != -1 ) {
			showViewElementColumn(osnEP);
			$("li:has(:contains('Контрагент'))").show();
		} else{
			hideViewElementColumn(osnEP);
		}
	}else{
		hideViewElementColumn(osnEP);
	}
...
var forinput = ['Ответственный отдела планирования', 'Филиал', 'Номер несостоявшейся процедуры', 'Ответственный за проведение закупки', 'Ответственный за заключение договора'];
	forinput.forEach(function(item, i, forinput) {
		$("input[name='"+item+"']").val('');
		$("input[name='"+item+"']").attr('value','');
...
var processSMPChekbox = function (){
	
	$(document).on('change', "input[name^='ItemTab-registerOKDP']", function (e) {
		var table = $("div[data-name='ItemTab']");
		table.find("div.table-edit-row[data-rowkey]").each(function () {
			var okpdElement = $(this).find("input[data-field-name^='ItemTab-registerOKDP-']")
			var value = okpdElement.val();
			var sposZak = $("input[name='registerSpZakup']").val();
			if(value && value.trim() !== "" && sposZak !== "202498" && sposZak !== "202506" && sposZak !== "3363" && sposZak !== "202507" && sposZak !== "6066" && sposZak !== "169") {			
				doesExistInDictionary(value).then(function(exists){
					setCheckbox(exists);
					waitingDialog.hide();
					}, function(error){
	...
	function doesExistInDictionary(query){
		var defer = jQuery.Deferred();
		findDictionaryItemByKey("Перечни ТРУ",query, null, function(data){
			if(data.data)
			defer.resolve(true);
			else if(data.error){
	...
	function setCheckbox(checked){
		//var checkbox = $("[name='registerUchZakSMP'][type='checkbox']");
		//checkbox.prop( "checked", "checked");
	}
	
};

var CheckAttach = function() {
	 
	$(".document-view-actions").find("a:contains('Опубликовать на ЭТП')").each(function() {
        var button = $(this);
		var onclickFunc = button.attr('onclick');
		button.attr('onclick', 'return false;');
		button.click(function(ev) {
			var izv = $("li:has(:contains('Проект договора'))").find(".badge").text();
			
			if(izv == 0){
				showCommonErrors("Необходимо прикрепить проект договора");
			}else{
				onclickFunc=onclickFunc.replace('event','ev');
				eval(onclickFunc);
			}
		});
...
var progectContact = function() {    
    var Status_view = $(".documentView-field-value[data-name='Статус']").text();
	var progect = $("li:has(:contains('Проект договора'))");	
    var  ChangeStatuses = [
		"Закупка завершена",
		"Закупка завершена(одна заявка)",
		"Закупка завершена, один победитель",
...
function changeOrgZa(){
	$(document).on('change', "input[name='registerOrgZa']", function() {
		//$("input[name='registerSpZakup']").clear();
		$("input[name='registerSpZakup']").val('');
		$("input[name='registerSpZakupName']").val('');
		$("input[name='registerSpZakup']").val('');
		$("input[data-field-name='registerSpZakup']").val('');
		$("input[data-field-name='registerZakElFormYN']").prop('checked', false);
		$("input[data-field-name='registerZakElForm']").prop('checked', false);
...
function heightDropdownZero(){
	$("input[type='text'][name='OrgZakupki']").prev("div.ui-tool-button.ui-tool-dropdown").height(0)
}
...
var copynamedz = function () {
	var CurrentUserName = $(".header-menu-user-name").text();
	var CurrentUserId;
	...
	$.getJSON(  getRelativeUrl(url) , function (data) {					
		$(data.children).each(function(index, row){
			if (row.name == CurrentUserName) { 																		
				...
				if((row.role.indexOf("Департамент закупок ЦХД") !=-1) || (row.role.indexOf("Департамент закупок") !=-1)) {
				$("input[data-field-name='DepSotr']").val(CurrentUserName);
				$("input[name='DepSotrId']").val(CurrentUserId);
				}
			}	
		})
	})
...
var proverkaNMCRub = function () {
		var registerNMCS = $("input[data-field-name='registerNMCS']").autoNumeric('get');
		var flag=$("input[name='Currency_kod']").val();
		var spzak=$("input[name='registerSpZakup']").val();
		
		if ((flag === "RUB") && (registerNMCS>15000000) && (spzak === "200611")) {
			showCommonErrors('Максимальная допустимая сумма НМЦ для данного способа закупки 15 млн. рублей.');
			$("input[data-field-name='registerNMCS']").clear();
			$("input[data-field-name^='ItemTab-summaTax-']").clear();
			$("input[data-field-name^='ItemTab-priceTax-']").clear();
		} else if ((flag === "RUB") && (registerNMCS>7000000) && (spzak === "200610")) {
			showCommonErrors('Максимальная допустимая сумма НМЦ для данного способа закупки 7 млн. рублей.');
			$("input[data-field-name='registerNMCS']").clear();
			$("input[data-field-name^='ItemTab-summaTax-']").clear();
			$("input[data-field-name^='ItemTab-priceTax-']").clear();
		}
...
var proverkaNMCValuta = function () {
		var NMCD = $("input[data-field-name='NMCD']").autoNumeric('get');
		var flag=$("input[name='Currency_kod']").val();
		var spzak=$("input[name='registerSpZakup']").val();
		
		if ((flag !== "RUB") && (NMCD>15000000) && (spzak === "200611")) {
			showCommonErrors('Максимальная допустимая сумма НМЦ для данного способа закупки 15 млн. рублей.');
			$("input[data-field-name='registerNMCS']").clear();
			$("input[data-field-name='NMCD']").clear();
			$("input[data-field-name^='ItemTab-summaTax-']").clear();
			$("input[data-field-name^='ItemTab-priceTax-']").clear();
		} else if ((flag !== "RUB") && (NMCD>7000000) && (spzak === "200610")) {
			showCommonErrors('Максимальная допустимая сумма НМЦ для данного способа закупки 7 млн. рублей.');
			$("input[data-field-name='registerNMCS']").clear();
			$("input[data-field-name='NMCD']").clear();
			$("input[data-field-name^='ItemTab-summaTax-']").clear();
...
var smallPurchEdit = function () {
	...
	if (registerSpZakupName.indexOf("у единственного") != -1){
		$("[data-field-name='noAddNoPublic']").readonly(false)
		if (NMCD<=500000){
		...
		$("[data-field-name='smallPurch']").prop("checked", false);
		$("[data-field-name='noAddNoPublic']").readonly()
	}
...
$(document).on('change', "input[data-field-name='CMP']", function (e) { 
	proverkaOKPD2forMSP();
/* 	if (!$(this).is(":checked")) {
		$("input[data-field-name^='ItemTab-registerOKDP-']").val('');
		$("input[name^='ItemTab-registerOKDP-']").val('');
		$("input[data-field-name^='ItemTab-OKDPUMSP-']").val('')
		$("input[name^='ItemTab-OKDPUMSP-']").val('');
	} */
})
...
$(document).on('change', "input[type='checkbox'][data-field-name='Zakupkanevplan']", function (e) { 
	if ($(this).is(":checked")) {
		$("input[type='checkbox'][data-field-name='noAddNoPublic']").prop('checked', true);
	}else{
		$("input[type='checkbox'][data-field-name='noAddNoPublic']").prop('checked', false);
	}
})

var proverkaOsnEP = function () {
	var osnEP=$("input[name='osnEP']").val();
	$("input[name='numberNesost']").inputmask({ mask: '99999999999', greedy: false });	
	$("input[name='numberNesost']").attr("data-parsley-pattern", "\\d{11}");
	if (osnEP === "26") {
		$("input[data-field-name='numberNesost']").closest(".column-container").show();
		$("[data-related-field=numberNesost]").show();
		$("input[data-field-name='numberNesost']").prop("required", true);
		$("[data-related-field=numberNesost]").addClass("label-required");	
	}
	else {
		$("input[data-field-name='numberNesost']").prop("required", false);
		$("[data-related-field=numberNesost]").removeClass("label-required");
		$("input[data-field-name='numberNesost']").closest(".column-container").hide();
		$("[data-related-field=numberNesost]").hide();
		$("input[name='numberNesost']").val('');
	}
}

$(document).on('change', "input[data-field-name='osnEP']", function (e) { 
	proverkaOsnEP();
})

var izmPPZ = function () {
	var regstatus=$("input[name='regstatus']").val();
	var longTermcategorie = $("input[data-field-name='longTermcategorie']");
		
	if (regstatus === "Внесение изменений" && longTermcategorie.is(":checked")) {
		SumSMPRub.CalculateSumma()
	}
}
...
var noAddNoPublicRedonly = function () {
	var registerSpZakupName = $("input[name='registerSpZakupName']").val();
	var registerOrgZa = $("input[name='registerOrgZa']").val();
	var NMCD = $("input[name='NMCD']").autoNumeric('get');
	if (registerSpZakupName.indexOf("у единственного") != -1){
...
var plannedAfterSecondYearLogic = function () {
	...
	var noAddNoPublic = $("input[name='noAddNoPublic']");
	...
	if (!correction.is(":checked")){
		if (plannedAfterSecondYear.is(":checked")){
			...
			month.val('');
			$("input[name='PlanMonthName']").val('');
			$("input[data-field-name='month']").clear()
			...
			registerSpZakup.val('');
			$("input[name='registerSpZakupName']").val('');
			$("input[data-field-name='registerSpZakup']").clear();
			...
			noAddNoPublic.prop("disabled", true);
			noAddNoPublic.prop("checked", false);
			...
			noAddNoPublic.prop("disabled", false);
...
var arendaLogik = function () {
	var arendaNed = $("input[name='arendaNed']");
	var zakCommUslug = $("input[name='zakCommUslug']");
	var zakEnergy = $("input[name='zakEnergy']");
	var registerSpZakup = $("input[name='registerSpZakup']");
	var registerSpZakupbutton = $("#registerSpZakup");
	if (arendaNed.is(":checked") || zakCommUslug.is(":checked") || zakEnergy.is(":checked")){
		registerSpZakupbutton.prop("disabled", true);
		registerSpZakup.val('3363');
		$("input[name='registerSpZakupName']").val('Закупка у единственного поставщика (подрядчика, исполнителя) (до 01.07.18)');
		$("input[data-field-name='registerSpZakup']").val('Закупка у единственного поставщика (подрядчика, исполнителя) (до 01.07.18)');
		WinnerHide();
		osnPB();
		osnEP();
		changeOKPDforMSP();
...
$(document).on('change', "input[name='arendaNed']", function (e) {
    arendaLogik();
});

$(document).on('change', "input[name='zakCommUslug']", function (e) {
    arendaLogik();
});

$(document).on('change', "input[name='zakEnergy']", function (e) {
    arendaLogik();
});

var bigDealLogik = function () {
	var soglMincif = $("input[name='soglMincif']");
	var NMCD = $("input[name='NMCD']").autoNumeric('get');
	if (Number(NMCD) > 4778700){
		soglMincif.prop("checked", true);
	}
	else{
		soglMincif.prop("checked", false);
	}
};

var bigDealLogikUZD = function () {
	...
	$.getJSON(  getRelativeUrl(url) , function (data) {					
		$(data.children).each(function(index, row){
			if (row.name == CurrentUserName) { 																		
				CurrentUserId = row.key;												
				if(row.role.indexOf("УЗД") ==-1 && row.role.indexOf("УЗД по договорной деятельности") ==-1 && row.role.indexOf("УЗД по планированию") ==-1 && row.role.indexOf("УЗД по проведению закупки") ==-1) {
					$("[name='soglMincif'][type='checkbox']").readonly(true)
					$("[name='plannedAfterSecondYear'][type='checkbox']").readonly(true)
					$("[name='Zakupkanevplan'][type='checkbox']").readonly(true)
					$("[name='noAddNoPublic'][type='checkbox']").readonly(true)
					$("button[id='VidZak']").prop("disabled", true);
				}
			}			
		});
	})
...
var ArendaView = function () {
	var flag1=$("div[data-name='Аренда недвижимости']").find("input[type='checkbox']");
	var flag2=$("div[data-name='Закупка коммунальных услуг']").find("input[type='checkbox']");
	if ($(flag1).attr("checked") || $(flag2).attr("checked")) {	
		$("div[data-name='Номер лота в извещении']").closest(".column-container").hide(); 
...
function CheckDocumentSave() {
	...
	form.on("beforeSubmit", function (args) {
		...
		var RukUprId = $("input[name='RukUprId']").val(); // ID Руководителя управления
		var RukInicId = $("input[name='RukInicId']").val(); // ID Руководителя отдела
		var AgreedUprav = $("input[data-field-name='AgreedUprav']"); // признак того, что ППЗ будет согласовать у руководителя руководителя управления
			...
			if(RukUprId && RukInicId) {
				// Если "Функциональный руководитель инициатора" отличен от "Владельца бюджета" то доп согласование Владельцем бюджета
				if (RukUprId != RukInicId) {
					AgreedUprav.prop('checked', true);
				}
				else{
					AgreedUprav.prop('checked', false);
				}
			}			
...
function filterOsnEp() {
	let eventName = "DicDialogOpened",
		dicName = "Основание закупки у ЕП";
	...
	buttons.each(function (index, btn) {
		...
		jBtn.on(eventName, function (event, args) {
			let VidZak = $("input[name='VidZak']").val();
...
$(document).on('change', "input[name='VidZak']", function (e) {
    var VidZak = $("input[name='VidZak']").val();
    var osnEP = $("input[name='osnEP']").val();
	if (VidZak=='Филиал' && osnEP=='12'){
		$("input[data-field-name='osnEP']").closest(".clearfix").find(".dict-display-field").val("");
		$("input[name='osnEP']").val('');
		$("textarea[data-field-name='osnEPName']").text('');
		$("textarea[data-field-name='osnEPName']").val('');
...
scopes.onRegister(copynamedz);
scopes.onRegister(setOKP2andOKVED2Code);
// scopes.onRegister(checkinPlanEIS);
scopes.onRegister(proverkaOKPD2forMSP);
scopes.onRegister(changeOKPDforMSP);
scopes.onRegister(heightDropdownZero);
scopes.onRegister(proverkaOsnEP);
scopes.onRegister(izmPPZ);
scopes.onRegister(godnered);
scopes.onRegister(checkboxnered);
// scopes.onRegister(DisabledIfCorrection);
scopes.onRegister(plannedAfterSecondYearLogic);
scopes.onRegister(arendaLogik);
scopes.onRegister(bigDealLogikUZD);
scopes.onRegister(Ed_IzmEditable);
scopes.onRegister(CheckDocumentSave);
scopes.onRegister(checkDepartment);
scopes.onRegister(filterOsnEp);

scopes.onEdit(changeOrgZa);
scopes.onEdit(processSMPChekbox);
scopes.onEdit(osnEP);
scopes.onEdit(osnPB);
scopes.onEdit(checklongtermcontract);
scopes.onEdit(SPZakrequired);
scopes.onEdit(nered);
...
// scopes.onEdit(checkinPlanEIS);
scopes.onEdit(proverkaOKPD2forMSP);
scopes.onEdit(changeOKPDforMSP);
scopes.onEdit(heightDropdownZero);
scopes.onEdit(proverkaOsnEP);
scopes.onEdit(godnered);
scopes.onEdit(checkboxnered);
scopes.onEdit(noAddNoPublicRedonly);
// scopes.onEdit(DisabledIfCorrection);
scopes.onEdit(plannedAfterSecondYearLogic);
scopes.onEdit(arendaLogik);
scopes.onEdit(bigDealLogikUZD);
scopes.onEdit(Ed_IzmEditable);
scopes.onEdit(CheckDocumentSave);
scopes.onEdit(filterOsnEp);